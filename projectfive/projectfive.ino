...
#define yellowBlinkTime 500 // 0.5 secs for yellow blink
...
void loop() {
  ...
  if (digitalRead(westButton) == HIGH) // request west>east trafficflow
  {
    if(trafficWest != true)
    {
      trafficWest = true; // change traffic flag to west> east
      delay(flowTime); // givr time for traffic to flow
  ...
  {
    if (trafficWest == true)// only continue if traffoc floe is in opposite (west) direction
    {
      trafficWest = false;// change traffic flow flag to east>west
      delay(flowTime);
      digitalWrite(westGreen, LOW);// change west lights from to yellow to red
      digitalWrite(westYellow, HIGH);
      delay(changeDelay);
      digitalWrite(westYellow, LOW);
      ...
      digitalWrite(eastYellow, LOW);
      digitalWrite(eastRed, LOW);// change east-facing lightd form red to greem
